      ...
      <div className="w-full max-w-7xl">
        ...
        <div className="flex flex-col lg:flex-row gap-8 mb-12">
          ...
          <div className="lg:w-1/2">
            <img  src={aboutImage}  alt="Living room with artwork"  className="w-full h-auto rounded-lg shadow-lg object-cover"/>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {frames.map((frame) => {
            ...
            return (
              <div key={frame.id} onClick={() => setSelectedFrame(frame.id)} className={`flex flex-col items-center p-6 rounded-lg transition-all duration-300 cursor-pointer ${selectedFrame === frame.id ? 'bg-[#E07A5F]' : 'bg-transparent border border-[#7A6E6E]'}`}>
                <div className={`w-12 h-12 flex items-center justify-center rounded-full mb-4 ${selectedFrame === frame.id ? 'bg-white' : 'bg-[#7A6E6E]'}`}>
                  <Icon className={`text-xl ${selectedFrame === frame.id ? 'text-[#7A6E6E]' : 'text-white'}`}/>
                </div>
                <h3 className={`text-2xl md:text-[32px] font-bold ${selectedFrame === frame.id ? 'text-white' : 'text-[#7A6E6E]'}`}>{frame.value}</h3>
                <p className={`text-center mt-2 ${selectedFrame === frame.id ? 'text-white' : 'text-[#7A6E6E]'}`}> {frame.label}</p>
              </div>
            );
          })}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 font-[Alexandria]">
            {teamMembers.map((member) => (
              <div key={member.id} className="flex flex-col items-center">
                <img src={member.image} alt={member.name} className="w-full h-96 object-cover rounded-lg shadow-lg"/>
                <h3 className="text-[#7A6E6E] text-2xl font-semibold mt-4">{member.name}</h3>
                <p className="text-[#7A6E6E] text-base mt-1">{member.role}</p>
